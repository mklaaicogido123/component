...
export const VeXemPhimReducer = (state = VeXemPhimState, action) => {
  switch (action.type) {
    case "CHOICED_SEAT": {
      let setUpdate = { ...state.arrGhe };
      setUpdate[action.y].danhSachGhe[action.x].daDat = true;
      state.arrGhe = setUpdate;
      console.log(setUpdate[action.y].danhSachGhe[action.x].daDat);
      return { ...state };
    }
    case "REMOVE_SEAT": {
      let setUpdate = { ...state.arrGhe };
      setUpdate[action.y].danhSachGhe[action.x].daDat = false;
      state.arrGhe = setUpdate;
      state.total -= setUpdate[action.y].danhSachGhe[action.x].gia;
      return { ...state };
    }
  }
